...
def create_dataset(data, time_step=60, forecast_day=7):
    ...
    for i in range(time_step, len(data) - forecast_day + 1):
        ...
        y.append(data[i:i + forecast_day, 0])
    return np.array(X), np.array(y)


def load_and_preprocess(time_step=60, forecast_day=7, min_data_points=120):
    df = load_raw_data()
...
if __name__ == "__main__":
    X_train, y_train, X_val, y_val, X_test, y_test, scaler = load_and_preprocess(forecast_day=7)
